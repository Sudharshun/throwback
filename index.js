...
Object.keys(questions).forEach((item) => {
    labels.push(blessed.text({
        parent: screen,
        top: vTop,
        left: 5,
        content: questions[item].label
    }));

    vTop++;

    questionTexts.push(blessed.textbox({
        parent: form,
        name: questions[item].name,
        top: vTop,
        left: 5,
        height: 3,
        inputOnFocus: true,
        content: questions[item].content,
        border: {
            type: 'line'
        },
        focus: {
...
form.on('submit', function (data) {
    msg.display('Form submitted'+data.firstName, function () {
        var summary = '';
